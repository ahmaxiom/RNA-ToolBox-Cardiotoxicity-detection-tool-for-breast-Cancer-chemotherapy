function  [All_masks,smoothedPoly, smoothedPoly1, max_img,min_img,imvolmx,imvolmn, bw, bw1, area1, area2, rate,minLongDIVmaxL, minShortDIVmaxL,AverageDiv, circularityMax, circularityMin,ElongationMax,ElongationMin,sp]=max_min_MUGA(Imag,meanBG)
...
imvolx=sum(imvol,3);
%figure(20), 
mb=msgbox("Pick a centered seed in this Image","Seed Point");
waitfor(mb);
%imshow(im2uint8(mat2gray(imvolx))) , hold all,colormap(jet),colorbar;
[poly, mask, smoothedPoly,sp] = regionGrowing2(im2uint8(mat2gray(imfilter(imvolx,[0 -1 0;-1 5 -1;0 -1 0]))),4,[],itrMx,itrMxRG);
waitfor(mask);
mask=uint16(mask);
for i=1:O
...
%% ROI max_img
[All_masks,bw, poly, smoothedPoly,bw1,poly1,smoothedPoly1]=ROI_max_min(Imag,max_img,min_img,meanBG,sp,imvol2); 
waitfor(bw);
%bw =uint16(bw);
%% ROI min_img
...
% uncomment 78-82 if you want to show max image during execution
%figure(3)
...
imvolmx=max_img;
imvolmx(bw==0)=0;
% uncomment 86-91 if you want to show max roi during execution
...
minLongDIVmaxL=(e1.long_axis-e2.long_axis)/e1.long_axis; % LA FS in excel sheet

minShortDIVmaxL=(e1.short_axis-e2.short_axis)/e1.short_axis;% SA FS in excel sheet

AverageDiv=(minLongDIVmaxL+minShortDIVmaxL)/2; % Average FS in excel sheet
%% max image
PerimeterMax=2*pi*sqrt(((e1.short_axis)^2+(e1.long_axis)^2)/2);
circularityMax=(4*pi*area1)/(PerimeterMax.^2); %Circulatory diastole in excel sheet
ElongationMax=e1.short_axis/e1.long_axis;      %Elongation diastole in excel sheet
%% min image
PerimeterMin=2*pi*sqrt((e2.short_axis^2+e2.long_axis^2)/2);
circularityMin=(4*pi*area2)/(PerimeterMin.^2); %Circulatory systole in excel sheet
ElongationMin=e2.short_axis/e2.long_axis;      %Elongation systole in excel sheet